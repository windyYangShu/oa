...
<!-- 设置el表达式有效，对于不同版本有些可以不用设置，但为了明显还是显式设置一下 -->
<%@ page isELIgnored="false" %>
<!-- 获取页面的根目录，额外使用ctx变量是因为引入js文件时 -->


<%-- 设置根目录，如果不设置的话根目录就会随着页面路径的变化而变化那么在不同页面得到的basePath便会不同
    设置完成后这个后面所有的href都有了一个基准URL就是basePath的路径，那么便可以在此基础上接着写了
    设置ctx是因为对于一些资源的引用src="路径"还是需要改全路径的，使用${basePath}这样就是错误的根本得不到路径信息
--%>
<base href="<%=basePath%>" >
<%-- 这个也是jsp特有的注释语句
    <%=输出语句表达式%> 这是jsp的输出语句，在后面有时用el表达式不太清除是可以直接使用jsp的表达式
--%>
<%--
    一开始很奇怪为什么上面还要加一句 request.setAttribute("ctx", basePath);难道直接使用${basePath}不可以吗？
    ${变量名}：会分别从page、request、session作用域内查找，看是否有符合该变量名的，当查找到便返回。相当于request.getAttribute("变量名")
--%>
...
<head>
    ...
    <!--  一开始是这样的连接形式href="assets/ ..."这样资源路径是写死的，Tomcat上稍微改变一下项目路径就会加载不出来 -->
...
<div id="main">
    ...
    <header class="navbar navbar-fixed-top navbar-shadow">
        <div class="navbar-branding">
            <!-- 在下面的自定义标签中测试El表达式是不正常工作的，如何解决暂时没有思路 -->
        ...
        <ul class="nav navbar-nav navbar-right">
            <li class="dropdown menu-merge">
                <a href="#" class="dropdown-toggle fw600 p15" data-toggle="dropdown">
                    <img src="${ctx}assets/img/avatars/5.jpg" alt="avatar" class="mw30 br64">
                ...
                <div class="sidebar-widget author-widget">
                    <div class="media">
                        <a class="media-left" href="#">
                            <img src="${ctx}assets/img/avatars/3.jpg" class="img-responsive">
                        </a>
    ...
    <section id="content_wrapper">
        <!-- 这里是故意没有把section和div填充完，为了使头页面包含下面的页面信息 -->
